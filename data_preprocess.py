...
def process_data():
    ...
    scene_names = ["sponza", "bistro"]

    POSITION_LIMIT_SQUARED = {"sponza":0.001600, "classroom":0.010000, "living-room":0.000900, "san-miguel":0.000900, "sponza-glossy":0.001600, "sponza-moving-light": 0.001600, "bistro": 0.001600}
    NORMAL_LIMIT_SQUARED = {"sponza":0.250000, "classroom":1.000000, "living-room":0.040000, "san-miguel":1.000000, "sponza-glossy":0.250000, "sponza-moving-light": 0.250000, "bistro": 0.250000}
    ...
    for scene_name in scene_names:
        ...
        for i in tqdm(range(60)):
            for h_i in range(H):
                for w_i in range(W):
                    world_position[:3] = world_positions[i][h_i, w_i][:3]
                    normal = normals[i][h_i, w_i][:3]
                    current_color = colors[i][h_i, w_i][:3]
                    ...
                    if i > 0:
                        ...
                        for j in range(4):
                            ...
                            if sample_location[0] >= 0 and sample_location[0] < W and sample_location[1] >= 0 and sample_location[1] < H:
                                prev_world_position = world_positions[i-1][sample_location[1], sample_location[0]][:3]
                                ...
                                if position_distance_squared < POSITION_LIMIT_SQUARED[scene_name]:
                                    prev_normal = normals[i-1][sample_location[1], sample_location[0]][:3]
